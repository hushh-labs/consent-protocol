...
function parseJsonOrToast<T extends Jsonish>(
  label: string,
  raw: string
): T | null {
...
export function InvestorProfileEditor(props: {
  ...
  const applyAdvancedJson = () => {
    ...
    const quotes = parseJsonOrToast<unknown[]>(
      "Public quotes",
      publicQuotesJson
    );
        ...
        <TabsContent value="portfolio" className="space-y-3">
          ...
          <div className="glass-interactive rounded-xl border border-border/50 p-4 space-y-3">
            ...
            <div className="space-y-2">
              {sectorRows.length === 0 && (
                <div className="text-xs text-muted-foreground">
                  Add sector weights (%) to help Kai flag concentration and
                  cyclicality risk.
                </div>
              )}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                <Input
                  ...
                  onChange={(e) => {
                    ...
                    onChange({
                      ...value,
                      last_13f_date: e.target.value || null,
                    });
                  }}

...
class SyncServiceImpl {
  ...
  async sync(): Promise<SyncResult> {
    ...
    if (settings.syncOnWifiOnly) {
      ...
      if (!isOnline) {
        ...
        return { success: true, synced: 0, failed: 0, errors: ['Network required'] };
      }
    }
  ...
  async pull(userId: string): Promise<{ updated: number }> {
    ...
    try {
      const response = await fetch(`/api/sync/pull?userId=${userId}&since=${this.lastSyncTime || 0}`);
      if (!response.ok) throw new Error(`Pull API error: ${response.status}`);

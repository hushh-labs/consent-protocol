...
function LoginScreenContent() {
  ...
  // State: "ready" (Login Buttons) or "vault" (Passphrase/Create/Unlock)
  const [view, setView] = useState<"ready" | "vault">("ready");
  const [vaultStep, setVaultStep] = useState<string>("checking"); // Track inner vault step for header logic
  const [error, setError] = useState<string | null>(null);

  // Helper to get current user since we need it for VaultFlow
  ...
  useEffect(() => {
    ...
    getRedirectResult(auth)
      .then(async (result) => {
        if (result?.user) {
          const hasVault = await VaultService.checkVault(result.user.uid);
          if (hasVault) {
    ...
    if (currentUser) {
      // Async check
      VaultService.checkVault(currentUser.uid).then((hasVault) => {
        if (hasVault) {
          router.push(redirectPath);
        } else {
          setView("vault");
  ...
  }, [router, redirectPath, currentUser]);

  const handleGoogleLogin = async () => {
    try {
      setError(null);
      await AuthService.signInWithGoogle();
      // On native/popup, if successful, switch view. Redirect handled above.
  ...
  return (
    <main className="flex-1 flex items-center justify-center p-6">
      <div className="w-full max-w-md space-y-6">
        {/* Header - Shown only for Login state. VaultFlow handles its own headers. */}
        ...
        {view === "ready" ? (
          ...
          currentUser && (
            <VaultFlow
              ...
              onStepChange={(step) => setVaultStep(step)}
            />
          )
        )}

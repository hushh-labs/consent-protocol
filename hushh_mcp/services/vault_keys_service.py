...
class VaultKeysService:
    ...
    async def setup_vault(
        ...
        supabase.table("vault_keys").upsert(data, on_conflict="user_id").execute()
    ...
    async def complete_onboarding(self, user_id: str) -> bool:
        ...
        if result.count == 0:
            logger.warning(
                f"⚠️ Onboarding complete attempted for {user_id} but NO ROW UPDATED. User ID mismatch?"
            )
            return False

        logger.info(
            f"✅ Onboarding marked complete for user {user_id[:8]} (Rows updated: {result.count})"
        )
        return True

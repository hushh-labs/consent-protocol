...
async def wait_for_consent_via_sse(
    user_id: str, request_id: str, scope: str, fastapi_url: str, timeout_seconds: int = 300
) -> ConsentResolution:
    """
    ...
    try:
        ...
        return ConsentResolution(
            status="error", request_id=request_id, message=f"Cannot connect to consent server: {e}"
        )
        ...
        return ConsentResolution(
            status="error", request_id=request_id, message=f"SSE error: {str(e)}"
        )

# api/routes/food.py
"""
Food Agent - Modular Backend with VAULT_OWNER Token Validation
...
router = APIRouter(prefix="/api/food", tags=["Food Agent"])
...
async def get_food_preferences(request: Request):
    ...
    Returns encrypted food preference data from vault.
    """
    try:
        ...
        # Fetch encrypted preferences from vault_food table
        pool = await get_pool()
        ...
        logger.error(f"❌ Error fetching food preferences: {e}")
        raise HTTPException(status_code=500, detail=str(e))


@router.post("/preferences/store")
async def store_food_preferences(request: Request):
    """
    Store food preferences for vault owner.
    
    Requires VAULT_OWNER consent token with write permissions.
    Stores encrypted food preference data to vault.
    """
    try:
        body = await request.json()
        user_id = body.get("userId")
        ...
        # Store encrypted preference in vault_food table
        pool = await get_pool()
        async with pool.acquire() as conn:
            # Upsert: update if exists, insert if not
            await conn.execute(
                """
                INSERT INTO vault_food (user_id, field_name, ciphertext, iv, tag, algorithm, created_at, updated_at)
        ...
        logger.info(f"✅ Food preference '{field_name}' stored for {user_id}")
        ...
        logger.error(f"❌ Error storing food preferences: {e}")
        raise HTTPException(status_code=500, detail=str(e))

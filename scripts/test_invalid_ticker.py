...
async def test_invalid_ticker():
    ...
    # Issue Token
    token = issue_token(
        user_id=user_id, agent_id="agent_kai", scope=ConsentScope("agent.kai.analyze")
    )

    # Initialize Orchestrator
    orchestrator = KaiOrchestrator(
        user_id=user_id, risk_profile="balanced", processing_mode="hybrid"
    )

    # Test with INVALID ticker
    print("\nTesting: INVALIDTICKER123")
    try:
        decision = await orchestrator.analyze(ticker="INVALIDTICKER123", consent_token=token.token)
